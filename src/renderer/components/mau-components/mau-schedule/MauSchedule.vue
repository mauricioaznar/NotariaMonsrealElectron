...
<script>
  import MauScheduleModal from './MauScheduleModal.vue'

  export default {
    name: 'MauSchedule',
    data () {
      return {
        divsCalculated: false,
        pageTimeGround: [],
        showModal: false,
        showModalDetail: {
          dateStart: '09:30',
    ...
    components: {
      Modal: MauScheduleModal
    },
    watch: {
      timeGround: function (timeGround) {
        this.pageTimeGround = this.initTimeGroud(timeGround)
      }
    },
    created () {
      // console.log(this.ta)
    ...
    mounted () {
      ...
      this.timeListSty.width = this.weekGround.length * 15 + '%'
      // console.log(this.taskDetail)
